...
static int sdl_windowwidth = 0;
static int sdl_windowheight = 0;
...
void sdl_bitblt_to_texture(int _x, int _y, int _w, int _h) {
  ...
  int dy = 0;
  Uint32 fg = SDL_MapRGB(sdl_pixelformat,0,0,0), bg = SDL_MapRGB(sdl_pixelformat,255,255,255);
  if( inverted_flag )
  {
      Uint32 tmp = fg;
      fg = bg;
  ...
  for (int sy = ystart; sy < ylimit; sy += sourcepitchwords, dy += dstpitchpixels) {
    ...
    for (int sx = xstart; sx < xlimit; sx++, dx += bitsperword) {
      ...
      for (int b = 0; b < bitsperword; b++) {
        ((Uint32 *)dst)[dy + dx + b] = (srcw & bitmask[b]) ? fg : bg;
      }
    }
  }
